...
<div class="w-full max-w-2xl space-y-6">
    ...
    <div class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
        ...
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            ...
            @if($donation->gateway)<div><dt class="text-slate-500 dark:text-slate-400">بوابة الدفع</dt><dd class="font-medium text-slate-800 dark:text-white">{{ strtoupper($donation->gateway) }}</dd></div>@endif
            @if($donation->gateway_status)<div><dt class="text-slate-500 dark:text-slate-400">حالة البوابة</dt><dd class="font-medium text-slate-800 dark:text-white">{{ $donation->gateway_status }}</dd></div>@endif
            @if($donation->gateway_transaction_id)<div><dt class="text-slate-500 dark:text-slate-400">رقم العملية الإلكترونية</dt><dd class="font-medium text-slate-800 dark:text-white">{{ $donation->gateway_transaction_id }}</dd></div>@endif
