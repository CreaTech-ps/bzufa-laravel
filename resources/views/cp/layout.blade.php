            ...
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 overflow-y-auto py-4 px-3">
                    <ul class="space-y-0.5">
                        ...
                        @if(cpCan('financial'))
                        {{-- المالية والتبرعات --}}
                        <li class="cp-nav-section pt-2 mt-2 {{ request()->routeIs('cp.financial.index', 'cp.donations.*', 'cp.financial-transactions.*', 'cp.financial.reports.*', 'cp.payment-gateway.*') ? '' : 'cp-collapsed' }}" data-section="financial">
                            <button type="button" class="cp-section-toggle" aria-expanded="{{ request()->routeIs('cp.financial.index', 'cp.donations.*', 'cp.financial-transactions.*', 'cp.financial.reports.*', 'cp.payment-gateway.*') ? 'true' : 'false' }}">
                            ...
                            <ul class="cp-section-content space-y-0.5">
                                ...
                                <li>
                                    <a href="{{ route('cp.payment-gateway.edit') }}" class="cp-nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-primary transition-colors {{ request()->routeIs('cp.payment-gateway.*') ? 'bg-primary/10 text-primary dark:bg-primary/20' : '' }}">
