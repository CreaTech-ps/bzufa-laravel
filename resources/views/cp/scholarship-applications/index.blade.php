...
@section('title', 'طلبات التقديم على المنح')

@section('content')
<div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 class="text-xl font-bold text-slate-800 dark:text-white">طلبات التقديم على المنح</h2>
        <a href="{{ route('cp.scholarships.index') }}" class="text-sm text-primary hover:underline">← العودة إلى المنح</a>
    </div>

    <form action="{{ route('cp.scholarship-applications.index') }}" method="get" class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 shadow-sm">
        <div class="flex flex-wrap items-end gap-3">
            <div class="min-w-[200px]">
                <label for="scholarship_id" class="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">المنحة</label>
                <select name="scholarship_id" id="scholarship_id" class="cp-input w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 px-3 py-2 text-sm focus:ring-2 focus:ring-primary/30">
                    <option value="">الكل</option>
                    @foreach($scholarships as $s)
                        <option value="{{ $s->id }}" {{ request('scholarship_id') == $s->id ? 'selected' : '' }}>{{ $s->title_ar }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-right">
                    <thead class="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            <th class="px-4 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">المنحة</th>
                            <th class="px-4 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">اسم مقدم الطلب</th>
                            <th class="px-4 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">رقم الهوية</th>
                            <th class="px-4 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">الملف</th>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @foreach($applications as $app)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                <td class="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">{{ $app->scholarship?->title_ar ?? '—' }}</td>
                                <td class="px-4 py-3 font-medium text-slate-800 dark:text-white">{{ $app->applicant_name }}</td>
                                <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-400">{{ $app->applicant_id_number }}</td>
                                <td class="px-4 py-3">
                                    <a href="{{ asset('storage/' . $app->file_path) }}" target="_blank" class="text-primary hover:underline text-sm">تحميل</a>
                                </td>
                                <td class="px-4 py-3">
                                    @php
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('cp.scholarship-applications.edit', $app) }}" class="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300" title="تعديل الحالة">
