...
@section('title', 'طلبات الشراكة تمكين')

@section('content')
<div class="space-y-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 class="text-xl font-bold text-slate-800 dark:text-white">طلبات الشراكة تمكين</h2>
    </div>

    <form action="{{ route('cp.tamkeen.partnership-requests.index') }}" method="get" class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 shadow-sm">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                        @foreach($requests as $req)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <a href="{{ route('cp.tamkeen.partnership-requests.edit', $req) }}" class="p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-600 dark:text-slate-300" title="تعديل الحالة">
